...
export default function FileList({ onFileSelect, selectedFile }: FileListProps) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      setUser(user);
      // No longer auto-load files - user must click "Connect to Google Drive" button
    });

    return () => unsubscribe();
  }, []);

  // Don't automatically load files - let user click "Connect to Google Drive" button
    ...
    return (
      <div className="p-6">
        <div className="text-center">
          <div className="mb-6">
            <div className="w-16 h-16 bg-white rounded-full mx-auto mb-4 flex items-center justify-center shadow-lg border-2 border-gray-100">
